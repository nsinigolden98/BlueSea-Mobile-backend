...
urlpatterns = [
    ...
    path("tickets/scan/", ScanTicketView.as_view(), name="scan-ticket"),
    path(
        "events/<uuid:event_id>/scan-stats/",
        ScannerDashboardView.as_view(),
        name="scanner-dashboard",
    ),
    path(
        "my-scanner-assignments/",
        MyScannerAssignmentsView.as_view(),
        name="my-scanner-assignments",
    ),
    path(
        "events/<uuid:event_id>/scanner/",
        AddEventScannerView.as_view(),
        name="add-event-scanner",
    ),
    # Withdrawal endpoints
    path(
        "verify-account-name/",
        VerifyAccountNameView.as_view(),
        name="verify-account-name",
    ),
    path("withdraw/", EventWithdrawalView.as_view(), name="withdraw"),
]

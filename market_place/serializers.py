        ...
        try:
            ...
            if not vendor.is_verified:
                raise serializers.ValidationError(
                    "Vendor must be verified to create events"
                )
            return value
...
class CreateEventSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if "is_free" in data and isinstance(data["is_free"], str):
            data["is_free"] = data["is_free"].lower() in ["true", "1", "yes"]
        ...
        if is_free:
            if not quantity or quantity <= 0:
                raise serializers.ValidationError(
                    {"quantity": "Quantity is required for free events"}
                )
            if ticket_types:
                raise serializers.ValidationError(
...
class PurchaseTicketSerializer(serializers.Serializer):
    ...
    quantity = serializers.IntegerField(
        required=True, min_value=1, max_value=10, help_text="Number of tickets (1-10)"
    )
        ...
        if not event.is_free:
            ...
            try:
                ...
                available_types = list(
                    TicketType.objects.filter(event=event).values_list(
                        "name", flat=True
                    )
                )
                raise serializers.ValidationError(
        ...
        if not attendees:
            if request and request.user.is_authenticated:
                ...
                if not full_name or full_name == "":
                    full_name = (
                        request.user.username or request.user.email.split("@")[0]
                    )

                user_attendee = {"name": full_name, "email": request.user.email}
...
class IssuedTicketSerializer(serializers.ModelSerializer):
    ...
    event_location = serializers.CharField(
        source="event.event_location", read_only=True
    )
    event_banner = serializers.SerializerMethodField()
    vendor_name = serializers.CharField(
        source="event.vendor.brand_name", read_only=True
    )
...
class TicketDetailSerializer(serializers.ModelSerializer):
    ...
    purchased_by_email = serializers.EmailField(
...
class TransferTicketSerializer(serializers.Serializer):
    ...
    recipient_email = serializers.EmailField(
        required=True, help_text="Recipient's email address"
    )
    recipient_name = serializers.CharField(
        required=True, max_length=255, help_text="Recipient's full name"
    )

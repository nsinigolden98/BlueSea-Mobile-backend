    ...
    def post(self, request):
            ...
            if ticket_types_data in ["[]", "", "null", "None"]:
                ...
                try:
                    ...
                    if not isinstance(ticket_types_data, list):
                        return Response(
                            {"error": "ticket_types must be an array", "state": False},
        ...
        if serializer.is_valid():
            try:
                ...
                response_serializer = EventInfoSerializer(
                ...
                return Response(
                    {"error": f"Failed to create event: {str(e)}", "state": False},
    ...
    def post(self, request):
        ...
        try:
            if not request.user.pin_is_set:
                logger.warning(
                    f"Vendor creation failed for user {request.user.id}: PIN not set"
                )
                return Response(
                    {
            ...
            if TicketVendor.objects.filter(user=request.user).exists():
                logger.warning(
                    f"Vendor creation failed for user {request.user.id}: Vendor already exists"
                )
                return Response(
            ...
            logger.debug(
                f"Extracted fields - business_type: {business_type}, brand_name: {brand_name}, "
            ...
            if missing_fields:
                logger.warning(
                    f"Vendor creation validation failed for user {request.user.id}: "
            ...
            logger.debug(
                f"File validation - id_document: {bool(id_document)}, "
            ...
            if not id_document:
                logger.warning(
                    f"Vendor creation validation failed for user {request.user.id}: ID document missing"
                )
                return Response(
            ...
            if not proof_of_address:
                logger.warning(
                    f"Vendor creation validation failed for user {request.user.id}: Proof of address missing"
                )
                return Response(
            ...
            if id_document.content_type not in allowed_types:
                ...
                return Response(
                    {
                        "error": "Invalid ID document format. Allowed: PDF, JPEG, PNG",
                        "success": False,
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            if proof_of_address.content_type not in allowed_types:
                ...
                return Response(
                    {
                        "error": "Invalid proof of address format. Allowed: PDF, JPEG, PNG",
                        "success": False,
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            logger.info(
                f"Creating vendor for user {request.user.id} - brand_name: {brand_name}, "
            ...
            logger.error(
                f"Vendor creation failed for user {request.user.id}: {type(e).__name__}: {str(e)}",
                exc_info=True,
            )
            return Response(
                {
...
class VendorStatusView(APIView):
    ...
    def get(self, request):
        try:
            ...
            return Response(
                {"error": "No vendor profile found", "success": False},
                status=status.HTTP_404_NOT_FOUND,
            )
...
class PurchaseTicketView(APIView):
    ...
    def post(self, request, event_id):
        ...
        serializer = PurchaseTicketSerializer(
            data=request.data, context={"request": request, "event_id": event_id}
        )
        ...
        if event.is_free:
            if ticket_type_obj:
                return Response(
                    {"error": "Free events do not have ticket types", "state": False},
                    status=status.HTTP_400_BAD_REQUEST,
                )

            # Check available quantity for free events
            available_quantity = event.quantity or 0
            tickets_sold = event.issued_tickets.exclude(status="canceled").count()
            remaining = available_quantity - tickets_sold

            if remaining <= 0:
                return Response(
                    {"error": "This event is fully booked", "state": False},
                    status=status.HTTP_400_BAD_REQUEST,
                )

            if quantity > remaining:
                return Response(
                    {"error": f"Only {remaining} ticket(s) available", "state": False},
                    status=status.HTTP_400_BAD_REQUEST,
                )
                ...
                with transaction.atomic():
                    ...
                    for attendee in attendees:
                        ...
                        try:
                            ...
                            logger.warning(
                                f"QR code image generation failed: {str(qr_error)}"
                            )
                    ...
                    return Response(
                        {
                            ...
                            "tickets": IssuedTicketSerializer(
                                issued_tickets, many=True, context={"request": request}
                            ).data,
                            "total_cost": "0.00",
                        },
                        status=status.HTTP_201_CREATED,
        ...
        try:
            ...
            return Response(
                {"error": "Wallet not found. Please contact support.", "state": False},
                status=status.HTTP_404_NOT_FOUND,
            )
        ...
        try:
            with transaction.atomic():
                ...
                for attendee in attendees:
                    ...
                    # Generate QR code
                    qr_data = (
                        f"{str(ticket.id)}:{str(ticket_type.id)}:{attendee['email']}"
                    )
                    ticket.qr_code = qr_data
                    ticket.save()
            ...
            return Response(
                {"error": f"Transaction failed: {str(e)}", "state": False},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
        ...
        request={
            "application/json": {
                ...
                "properties": {
                    "qr_data": {
                        "type": "string",
                        "description": "QR code data from scanner",
                    },
                    "event_id": {
                        "type": "string",
                        "format": "uuid",
                        "description": "Event UUID being scanned for",
                    },
                },
            ...
            with transaction.atomic():
                ...
                if ticket.status == "used":
                    ...
                    scanned_by_info = {
                        ...
                        "time": ticket.scanned_at.strftime("%Y-%m-%d %H:%M:%S")
                    ...
                    return Response(
                        {
                            ...
                            "ticket_details": {
                                ...
                                "transferred_at": ticket.transferred_at.strftime(
                                    "%Y-%m-%d %H:%M:%S"
                                )
                    ...
                    return Response(
                        {
                            ...
                            "ticket_details": {
                                "ticket_id": str(ticket.id),
                                "canceled_at": ticket.canceled_at.strftime(
                                    "%Y-%m-%d %H:%M:%S"
                                )
                    ...
                    return Response(
                        {
                            ...
                            "scan_result": "rejected",
                            "event_start": ticket.event.event_date.strftime(
                                "%Y-%m-%d %H:%M:%S"
                            ),
                        },
                        status=status.HTTP_400_BAD_REQUEST,
                ...
                return Response(
                    {
                        ...
                        "ticket_details": {
                            ...
                            "status": ticket.status,
                            "scanned_at": ticket.scanned_at.strftime(
                                "%Y-%m-%d %H:%M:%S"
                            ),
    ...
    @extend_schema(
        ...
        parameters=[
            ...
            OpenApiParameter(
                ...
                type=str,
                location=OpenApiParameter.QUERY,
                description="Export format: csv or txt",
            ),
        ],
        responses={
            200: OpenApiTypes.OBJECT,
    ...
    def get(self, request, event_id):
        ...
        if export_format == "csv":
            ...
            response["Content-Disposition"] = (
                f'attachment; filename="attendees_{event.event_title}_{timezone.now().strftime("%Y%m%d")}.csv"'
            )

            writer = csv.writer(response)
            writer.writerow(
                ["Name", "Email", "Ticket Type", "Status", "QR Code", "Purchase Date"]
            )
            ...
            response["Content-Disposition"] = (
                f'attachment; filename="attendees_{event.event_title}_{timezone.now().strftime("%Y%m%d")}.txt"'
            )
            ...
            for ticket in tickets:
                ...
                lines.append(
                    f"Purchase Date: {ticket.created_at.strftime('%Y-%m-%d %H:%M:%S')}"
                )
                lines.append("-" * 80)
...
class MyTicketsListView(APIView):
    ...
    def get(self, request):
        ...
        return Response(
            {"state": True, "stats": stats, "tickets": serializer.data},
            status=status.HTTP_200_OK,
        )
...
class TicketDetailView(APIView):
    ...
    def get(self, request, ticket_id):
        ...
        if ticket.owner_email != request.user.email:
            return Response(
                {"error": "You do not own this ticket", "state": False},
                status=status.HTTP_403_FORBIDDEN,
            )
...
class TransferTicketView(APIView):
    ...
    def post(self, request, ticket_id):
        ...
        if recipient_email.lower() == request.user.email.lower():
            return Response(
                {"error": "You cannot transfer a ticket to yourself", "state": False},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if not can_transfer:
            return Response(
                {"error": message, "state": False}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            return Response(
                {"error": f"Transfer failed: {str(e)}", "state": False},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def post(self, request, ticket_id):
        ...
        if ticket.ticket_type and not ticket.event.is_free:
            ...
            try:
                ...
                return Response(
                    {"error": "Wallet not found", "state": False},
                    status=status.HTTP_404_NOT_FOUND,
                )
        ...
        try:
            with transaction.atomic():
                ...
                if refund_amount > 0 and ticket.ticket_type:
                    ...
                    logger.info(
                        f"Free ticket {ticket.id} canceled by user {request.user.id}"
                    )
...
class ScannerDashboardView(APIView):
    ...
    def get(self, request, event_id):
        try:
            ...
            return Response(
                {"error": "Event not found or not approved", "state": False},
                status=status.HTTP_404_NOT_FOUND,
            )
        ...
        is_authorized = (
            request.user.is_staff
            or EventScanner.objects.filter(user=request.user, event=event).exists()
            or event.vendor.user == request.user
        )
        ...
        recent_scans = (
            IssuedTicket.objects.filter(event=event, status="used")
            .select_related("scanned_by", "ticket_type")
        ...
        return Response(
            {
                ...
                "statistics": {
                    ...
                    "scan_percentage": round(
                        (total_scanned / total_issued * 100) if total_issued > 0 else 0,
                        2,
                    ),
                },
                "personal_stats": {
                    ...
                    "percentage_of_total": round(
                        (personal_scans / total_scanned * 100)
                        if total_scanned > 0
                        else 0,
                        2,
                    ),
...
class MyScannerAssignmentsView(APIView):
    ...
    def get(self, request):
        ...
        assignments = EventScanner.objects.filter(user=request.user).select_related(
            "event", "event__vendor"
        )
        ...
        for assignment in assignments:
            ...
            scanned_tickets = IssuedTicket.objects.filter(
                event=event, status="used"
            ).count()
        ...
        return Response(
            {"state": True, "count": len(assigned_events), "events": assigned_events},
            status=status.HTTP_200_OK,
        )
...
class AddEventScannerView(APIView):
    ...
    def post(self, request, event_id):
        ...
        try:
            ...
            scanner = EventScanner.objects.create(user=scanner_user, event=event)

            # Update user role to 'scanner' if they are a regular 'user'
...
class VendorTicketsList(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            if not vendor.is_verified:
                return Response(
                    {"error": "Your vendor account is not verified", "state": False},
                    status=status.HTTP_403_FORBIDDEN,
                )

        except TicketVendor.DoesNotExist:
            return Response(
                {"error": "Vendor profile not found", "state": False},
                status=status.HTTP_404_NOT_FOUND,
            )
        ...
        base_tickets = (
            IssuedTicket.objects.filter(event__vendor=vendor)
            .select_related("event", "ticket_type", "purchased_by", "scanned_by")
            .order_by("-created_at")
        )

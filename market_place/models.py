...
class TicketVendor(models.Model):
    ...
    business_type = models.CharField(
        max_length=20, choices=BUSINESS_TYPE_CHOICES, null=True, blank=True
    )
    legal_full_name = models.CharField(
        max_length=255,
        help_text="Legal full name from government ID",
        null=True,
        blank=True,
    )
    ...
    id_type = models.CharField(
        max_length=50,
        help_text="NIN, Passport, Driver's License",
        null=True,
        blank=True,
    )
    id_document = models.FileField(
        ...
        help_text="Upload ID document (PDF, JPEG, or PNG format)",
        null=True,
        blank=True,
    )
    proof_of_address = models.FileField(
        ...
        help_text="Utility bill or bank statement within last 3 months",
        null=True,
        blank=True,
    )
    ...
    categories = models.CharField(
        max_length=500,
        help_text="Comma-separated list of categories: concerts, conferences, religious, sports, others",
        null=True,
        blank=True,
    )
    monthly_volume = models.CharField(
        max_length=20,
        help_text="Estimated monthly ticket volume range",
        null=True,
        blank=True,
    )
    business_description = models.TextField(
        help_text="Description of business model and typical events",
        null=True,
        blank=True,
    )
...
class EventInfo(models.Model):
    ...
    quantity = models.IntegerField(
        null=True, blank=True, help_text="Total quantity for free events"
    )
...
class TicketType(models.Model):
    ...
    event = models.ForeignKey(
        EventInfo, on_delete=models.CASCADE, related_name="ticket_types"
    )
    name = models.CharField(
        max_length=255, default="General Admission"
...
class IssuedTicket(models.Model):
    ...
    event = models.ForeignKey(
        EventInfo, on_delete=models.CASCADE, related_name="issued_tickets"
    )
    ...
    qr_code_image = models.ImageField(
        upload_to="ticket_qr_codes/", null=True, blank=True
    )
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default="upcoming")

    # Transfer tracking
    transferred_to = models.EmailField(
        null=True,
        blank=True,
...
class EventScanner(models.Model):
    ...
    event = models.ForeignKey(
        EventInfo, on_delete=models.CASCADE, related_name="scanners"
    )
    created_at = models.DateTimeField(auto_now_add=True)

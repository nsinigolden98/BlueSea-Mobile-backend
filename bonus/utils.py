...
def award_points(
    user, points, reason, description, reference=None, metadata=None, created_by=None
):
    """
    ...
    try:
        with transaction.atomic():
            ...
            if bonus_account.points < points:
                raise ValueError(
                    f"Insufficient points. You have {bonus_account.points} points."
                )
...
def award_referral_bonus(referrer, referred_user):
    ...
    """
    # Referral bonus set to 50
    REFERRAL_BONUS = 50

    try:
        referral = Referral.objects.get(
            referrer=referrer, referred_user=referred_user, status="pending"
        )
...
def award_signup_bonus(user):
    """
    Award signup bonus if user was referred
    """
    SIGNUP_BONUS = 20
    ...
    try:
        ...
        # Check if signup bonus already awarded (via metadata check)
        ...
        logger.error(f"Error awarding signup bonus: {str(e)}")
        return None
    ...
    try:
        with transaction.atomic():
            ...
            if not bonus_account.can_claim_daily_login():
                logger.info(
                    f"User {user.email} already claimed daily login bonus today"
                )
                return None
...
def user_points_summary(user):
    try:
        ...
        # Get referral stats even if bonus account doesn't exist
        total_referrals = Referral.objects.filter(referrer=user).count()
        completed_referrals = Referral.objects.filter(
            referrer=user, status="completed"
        ).count()

...
class AutoTopUpCreateView(APIView):
    ...
    @extend_schema(
        ...
        responses={201: AutoTopUpSerializer, 400: OpenApiTypes.OBJECT},
        examples=[
            OpenApiExample(
                "Airtime Auto Top-Up",
                value={
    ...
    def post(self, request):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            try:
                service_label = (
                    "Airtime" if auto_topup.service_type == "airtime" else "Data"
                )
                frequency = (
                    "one-time"
                    if auto_topup.repeat_days == 0
                    else f"every {auto_topup.repeat_days} days"
                )
                ...
                send_notification(
                    ...
                    context={
                        ...
                        "network": auto_topup.network.upper()
                        if auto_topup.network
                        else "N/A",
...
class GetUserAutoTopUpsView(APIView):
    ...
    def get(self, request):
        auto_topups = (
            AutoTopUp.objects.filter(user=request.user)
...
class AutoTopUpDetailView(APIView):
    ...
    @extend_schema(
        ...
        responses={200: AutoTopUpDetailSerializer, 404: OpenApiTypes.OBJECT},
        tags=["Auto Top-Up"],
    )
    ...
    @extend_schema(
        ...
        responses={200: AutoTopUpSerializer, 404: OpenApiTypes.OBJECT},
        tags=["Auto Top-Up"],
    )
    @transaction.atomic
...
class AutoTopUpCancelView(APIView):
    ...
    def patch(self, request, pk):
        try:
            auto_topup = AutoTopUp.objects.select_related("user__wallet").get(
        ...
        return Response(
            {"error": "Failed to unlock funds"}, status=status.HTTP_400_BAD_REQUEST
        )
...
class AutoTopUpReactivateView(APIView):
    ...
    def patch(self, request, pk):
        ...
        return Response(
            {"error": "Failed to lock funds"}, status=status.HTTP_400_BAD_REQUEST
        )
